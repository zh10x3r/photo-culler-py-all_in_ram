...
右侧控制栏与预览区域之间的分隔线可以用鼠标拖动，以调整控制栏宽度；软件会记住下次启动时的宽度。控制项现在位于可纵向滚动的菜单中，窗口较矮或展开 GPU 设置时可以滚动查看全部按钮；GPU 设置默认收起，点击“GPU 预览设置”后展开。
...
主窗口中央的大图就是 PySide6 + VisPy/OpenGL GPU 预览，右侧“GPU 预览设置”按钮点击后才展开详细选项。主界面使用 Qt 的完整事件循环，VisPy 相机动画不再依赖 Tk→Qt 的事件泵，因此滚轮缩放、拖动平移和连续翻图共用同一套渲染路径。
